...
interface CreatePartnerQuotationProps {
  rolePrefix: string;
  partnerCode: string;
  partnerName: string;
...
export default function CreatePartnerQuotation({
  rolePrefix,
  partnerCode,
  partnerName,
  trigger
}: CreatePartnerQuotationProps) {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <Dialog.Title className="text-xl font-bold text-slate-900 tracking-tight">
                  Partner <span className="text-blue-600">Quotation</span>
                </Dialog.Title>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                  Target: <span className="text-slate-900">{partnerName}</span>
                </p>
              </div>
            </div>
          ...
          <Formik
            ...
            onSubmit={async (values, { setSubmitting }) => {
              try {
                ...
                toast.success("Partner quotation initialized successfully");
                queryClient.invalidateQueries({ queryKey: ["partners", partnerCode] });
                
                // Navigate to the quotation detail page
                router.push(`/${rolePrefix}/partners/${partnerCode}/${data.reference}`);
              } catch (error: any) {
            ...
            {({ isSubmitting, values }) => (
              <Form className="flex flex-col">
                <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto">
                  ...
                  <div className="space-y-3">
                    ...
                    <Field
                      as="textarea"
                      name="notes"
                      placeholder="Detail the scope or any specific terms for this partner proposal..."
                  ...
                  <div className="p-4 rounded bg-slate-50 border border-slate-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Official Merchant ID</p>
                    </div>
                    <p className="text-[10px] font-mono font-bold text-slate-400">{partnerCode}</p>
                  </div>
                </div>
                ...
                <div className="p-8 bg-slate-50 border-t border-slate-100 flex items-center justify-end">
                  ...
                  >
                    {isSubmitting ? "Syncing..." : "Initialize Proposal"}
