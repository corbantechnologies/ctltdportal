            ...
            <div className="bg-slate-900 px-5 py-4 text-white relative">
              ...
              <div className="flex items-center gap-3 relative z-10">
                ...
                <div>
                  ...
                  <p className="text-slate-400 font-semibold uppercase text-[9px] tracking-widest">
              ...
              <form onSubmit={formik.handleSubmit} className="space-y-4">
                      ...
                      {productTypes.map((type) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "h-8 sm:h-9 rounded border text-[10px] font-bold uppercase transition-all",
                              isSelected ? "bg-blue-600 border-blue-600 text-white shadow-sm" : "bg-slate-50 border-slate-200 text-slate-400 hover:border-slate-300"
                            )}
                          >
                            {type.label}
                          </button>
                      ...
                      {billingCycles.map((cycle) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "h-8 sm:h-9 rounded border text-[9px] font-bold uppercase transition-all text-center leading-tight px-1",
                              isSelected ? "bg-slate-900 border-slate-900 text-white shadow-sm" : "bg-slate-50 border-slate-200 text-slate-400 hover:border-slate-300"
                            )}
                          >
                            {cycle.label}
                          </button>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full h-9 sm:h-10 rounded border border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 transition-all font-medium pl-9 pr-3 text-xs sm:text-sm text-slate-900 outline-none disabled:opacity-50"
                        onChange={formik.handleChange}
                        onBlur={formik.handleBlur}
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    name="description"
                    rows={3}
                    className="w-full rounded border border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 transition-all font-medium p-3 text-xs sm:text-sm text-slate-900 outline-none resize-none min-h-[75px]"
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                ...
                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded border border-slate-100 hover:border-slate-200 transition-colors group cursor-pointer" onClick={() => formik.setFieldValue('is_active', !formik.values.is_active)}>
                  <div className={cn("w-5 h-5 rounded border-2 flex items-center justify-center transition-all", formik.values.is_active ? "bg-blue-600 border-blue-600 shadow-sm" : "bg-white border-slate-200")}>
                    {formik.values.is_active && <Plus className="w-3.5 h-3.5 text-white rotate-45" />}
                  </div>
                  <div>
                    <span className="text-xs font-bold text-slate-900 block">Operational Readiness</span>
                    <p className="text-[10px] font-medium text-slate-400 tracking-tight">Maintain market visibility</p>
                  </div>
                </div>
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full h-9 sm:h-10 bg-slate-900 hover:bg-blue-600 text-white rounded font-semibold text-xs sm:text-sm transition-all shadow-sm active:scale-[0.98] group flex items-center justify-center gap-2"
                  >
                    {formik.isSubmitting ? (
