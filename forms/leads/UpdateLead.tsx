        ...
        <div className="p-4 sm:p-5 max-h-[75vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200">
          <form onSubmit={formik.handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5 border-t border-slate-100 pt-4">
                <label htmlFor="status" className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 ml-1 block">
                    ...
                    {statusOptions.map((option) => {
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    "flex items-center justify-between p-2 sm:p-2.5 rounded border transition-all text-left",
                                    isSelected 
                                        ? "bg-blue-50 border-blue-200 shadow-sm" 
                                        : "bg-white border-slate-200 hover:border-slate-300"
                                )}
                            >
                                <span className={cn("text-[10px] sm:text-[11px] font-bold uppercase tracking-tight", isSelected ? "text-blue-700" : "text-slate-600")}>
                                    {option.label}
                                </span>
            ...
            {formik.values.status === "QUALIFIED" && lead.status !== "QUALIFIED" && (
                <div className="space-y-3 animate-in slide-in-from-top-2">
                    <div className="flex gap-2.5 p-3 bg-emerald-50 border border-emerald-100 rounded">
                    ...
                    <div className="space-y-1.5">
                        <label htmlFor="partner_type" className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 ml-1 block">
                            Partner Category Classification
                        </label>
                        <select
                            id="partner_type"
                            name="partner_type"
                            className="w-full h-9 sm:h-10 rounded border border-emerald-100 bg-emerald-50/30 focus:bg-white focus:border-emerald-500 transition-all font-medium px-3 text-xs sm:text-sm text-slate-900 outline-none appearance-none cursor-pointer"
                            onChange={formik.handleChange}
                            onBlur={formik.handleBlur}
