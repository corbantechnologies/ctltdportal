                    ...
                    <div>
                        ...
                        <div className="space-y-4 md:space-y-1">
                            {data.trial_balance.map((row) => (
                                <div
                                    key={row.code}
                                    onClick={() => setSelectedBookCode(row.code)}
                                    className="group grid grid-cols-12 gap-2 py-2 text-sm border-b border-border/40 hover:bg-corporate-secondary/50 cursor-pointer rounded-lg px-2 transition-colors"
                                    title="Click to view underlying transactions"
                                >
                                    <div className="col-span-12 md:col-span-6 flex items-center tracking-tight text-foreground" title={row.name}>
                                        <span className="truncate mr-1 max-w-[200px] sm:max-w-none group-hover:text-corporate-primary font-medium transition-colors">
                                            {row.name}
                                        </span>
                                        <span className="text-[10px] hidden md:inline-block text-muted-foreground">- {row.code}</span>
                                        <span className="text-[10px] md:hidden ml-1 opacity-50">({row.code})</span>
                                        <ExternalLink className="w-3 h-3 ml-1.5 opacity-0 group-hover:opacity-70 text-corporate-primary transition-opacity" />
                                    </div>
                        ...
                        <div className="grid grid-cols-12 gap-2 mt-2 -mx-2 px-6 rounded bg-corporate-primary/5 text-[11px] py-4 border border-corporate-primary/10 shadow-inner">
                            <div className="col-span-12 md:col-span-6 mb-2 md:mb-0 flex items-center text-corporate-primary font-semibold uppercase tracking-wider">
                                TOTAL DISCLOSURE
                            </div>
                            <div className="col-span-12 md:col-span-6 flex flex-col gap-1 md:contents mt-2 md:mt-0">
                                ...
                                <div className="flex justify-between items-center md:block md:col-span-2 text-right font-mono text-corporate-primary font-bold">
